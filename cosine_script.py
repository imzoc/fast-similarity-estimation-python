...
for e in range(num_edits):
    ...
    for k in k_range:
        est_dist = mh.kmer_vec_cosine_sim(s1, s2, k)
...
def plot():
    ...
    plt.ylabel('Cosine Similarity Between k-mer Frequency Vectors', fontdict=fontdict)
    plt.title(f'Figure 2: Cosine Similarity. String length: {seq_len}.', fontdict=fontdict)
    plt.legend()
    plt.grid(True)
    plt.show()
