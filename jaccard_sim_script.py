...
for e in range(num_edits):
    ...
    for k in k_range:
        est_dist = mh.kmer_jaccard_sim(s1, s2, k)
...
def plot():
    ...
    plt.ylabel('Jaccard Similarity Between k-mers', fontdict=fontdict)
    plt.title(f'Figure 3: Jaccard Similarity. String length: {seq_len}.', fontdict=fontdict)
    plt.legend()
    plt.grid(True)
    plt.show()
